...
function pbu.openssl.decode() {
  ...
  pbu.eval -v -- openssl "$type" -inform "$inform" -in "$file" -text -noout
}
...
function openssl-decode-asn1() {
  ...
  pbu.eval -v -- openssl asn1parse -inform "$inform" -in "$file"
}
...
function openssl-signature-extract() {
  ...
  pbu.eval -v -- openssl asn1parse -inform "$inform" -in "$file" -out "$outfile" -strparse "$x" -noout
}
...
function openssl-convert-x509-pem-to-der() {
  ...
  pbu.eval -v -- openssl x509 -inform pem -in "$file" -outform der -out $outfile
}
...
function openssl-publickey-extract() {
  ...
  pbu.eval -v -- openssl x509 -pubkey -inform "$inform" -in "$file" -outform "$outform" -out "$outfile" -noout
}
