...
alias update-pc-bash-utils='export ___PBU_UPDATE_TRIGGERED___=true && pushd ~/.pc_bash_utils > /dev/null && git pull && popd > /dev/null && echo "0" > ~/.pc_bash_utils/.updatepending && {  rm -r ~/.pc_bash_utils/bin > /dev/null; source ~/.pc_bash_utils/init.sh; }'
