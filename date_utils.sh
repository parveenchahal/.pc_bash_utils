...
function date-from-epoch() {
  ...
  local tz='local'
  pbu_is_arg_switch_enabled '' 'out-utc' "$@" && tz='utc'
...
function date-to-epoch() {
  pbu_is_arg_switch_enabled '' 'out-nanoseconds' "$@" ||
  pbu_is_arg_switch_enabled '' 'out-microseconds' "$@" ||
  pbu_is_arg_switch_enabled '' 'out-milliseconds' "$@" ||
  pbu_is_arg_switch_enabled '' 'out-seconds' "$@" ||
  ...
  pbu_is_arg_switch_enabled '' 'out-nanoseconds' "$@" && values+=("$input")  
  pbu_is_arg_switch_enabled '' 'out-microseconds' "$@" && values+=( $(($input / 1000)) )
  pbu_is_arg_switch_enabled '' 'out-milliseconds' "$@" && values+=( $(($input / 1000000)) ) 
  pbu_is_arg_switch_enabled '' 'out-seconds' "$@" && values+=( "$(($input / 1000000000))" )
